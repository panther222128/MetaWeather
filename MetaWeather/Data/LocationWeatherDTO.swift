//
//  ConsolidatedWeatherResponseDTO.swift
//  MetaWeather
//
...
struct LocationWeatherResponseDTOs: Decodable {
    ...
    func convertToDomain() -> LocationWeathers {
        let transformed = self.locationWeatherResponseDTOs.map( { $0.convertToDomain() } )
        return .init(locationWeathers: transformed, title: title)
    }
